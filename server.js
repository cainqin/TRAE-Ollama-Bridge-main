...
// OpenAI-compatible: list models (shared handler)
function handleListModels(req, res) {
  const mappings = readMappings();
  const created = Math.floor(Date.now() / 1000);
  const data = mappings.map(m => ({ id: m.id, object: 'model', created, owned_by: 'ollama' }));
  res.json({ object: 'list', data });
}

// OpenAI-compatible: get model detail (shared handler)
function handleGetModel(req, res) {
  const modelId = req.params.model;
  const mappings = readMappings();
  const mapping = mappings.find(m => m.id === modelId);
  if (!mapping) {
    return res.status(404).json({ error: { message: `Model '${modelId}' not found`, type: 'model_not_found' } });
  }
  const created = Math.floor(Date.now() / 1000);
  res.json({ id: mapping.id, object: 'model', created, owned_by: 'ollama', root: mapping.id });
}

app.get('/v1/models', authMiddleware, handleListModels);
app.get('/models', authMiddleware, handleListModels);
app.get('/v1/models/:model', authMiddleware, handleGetModel);
app.get('/models/:model', authMiddleware, handleGetModel);
...
function toOllamaChatPayload(body) {
  ...
  // normalize content: OpenAI may send array of parts, Ollama needs plain string
  function normalizeContent(content) {
    if (typeof content === 'string') return content;
    if (Array.isArray(content)) {
      return content.map(p => p.text || p.image_url?.url || '').join('\n');
    }
...
// OpenAI-compatible: POST chat completions (shared handler)
async function handleChatCompletions(req, res) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        writeSSE(res, {
          ...
          created,
          model: modelId,
          choices: [{ index: 0, delta: { content: `\n\n[Ollama 错误 ${r.status}] ${errBody || '请求失败，请确认模型名称是否正确'}` }, finish_reason: null }],
        });
        res.write('data: [DONE]\n\n');
        res.end();
        return;
      ...
      writeSSE(res, {
        ...
        created,
        model: modelId,
        choices: [{ index: 0, delta: { content: `\n\n[连接错误] 无法连接到 Ollama，请确认 Ollama 服务是否在运行 (${OLLAMA_BASE_URL})` }, finish_reason: null }],
      });
      res.write('data: [DONE]\n\n');
      res.end();
    }
  ...
  try {
    ...
    if (!r.ok) {
      const errBody = await r.text().catch(() => '');
      return res.status(502).json({ error: `Ollama 返回错误 (${r.status}): ${errBody}` });
    }
    const j = await r.json();
    ...
    res.status(502).json({ error: `Ollama 连接失败: ${e.message || e}` });
  }
}

app.post('/v1/chat/completions', authMiddleware, handleChatCompletions);
app.post('/chat/completions', authMiddleware, handleChatCompletions);

// Server-side proxy for intercept testing (browser -> HTTP -> server -> HTTPS -> self)
const interceptAgent = new https.Agent({
  ca: fs.existsSync(CA_CERT_FILE) ? fs.readFileSync(CA_CERT_FILE) : undefined,
  rejectUnauthorized: true,
});
app.post('/bridge/proxy/chat/completions', authMiddleware, async (req, res) => {
  try {
    const body = req.body || {};
    ...
    if (!r.ok) {
      return res.status(r.status).json({ error: `上游返回 ${r.status}: ${await r.text().catch(()=>'')}` });
    }
    if (body.stream) {
      res.set({
        'Content-Type': 'text/event-stream',
...
function start() {
  const bindHost = BIND_ADDRESS || '127.0.0.1';

  // Always start HTTP on PORT+2000 for WebUI access (avoid QQ port 4001)
  const httpPort = PORT + 2000;
  http.createServer(app).listen(httpPort, bindHost, () => {
    console.log(`TRAE-Ollama-Bridge WebUI on http://${bindHost}:${httpPort}/`);
  });
  ...
  if (HTTPS_ENABLED && fs.existsSync(SSL_CERT_FILE) && fs.existsSync(SSL_KEY_FILE)) {
    ...
    if (fs.existsSync(CA_CERT_FILE)) {
      const caCert = fs.readFileSync(CA_CERT_FILE, 'utf8').trim();
      if (!opts.cert.toString('utf8').trim().endsWith(caCert)) {
        opts.cert = opts.cert + '\n' + caCert;
      }
    }
    const server = https.createServer(opts, app);
    server.listen(PORT, bindHost, () => {
      console.log('TRAE-Ollama-Bridge HTTPS listening on https://' + bindHost + ':' + PORT + '/ (443->' + PORT + ' via portproxy if enabled)');
    });
  } else {
    const server = http.createServer(app);
    server.listen(PORT, bindHost, () => {
      console.log(`TRAE-Ollama-Bridge HTTP listening on http://${bindHost}:${PORT}/`);
    });
  }
}
